<div class="header">
    ...
    <nav class="bg-white px-6 py-[10px] flex items-center justify-between shadow-sm">
        ...
        <div class="ml-8 hidden md:hidden lg:block">
            @php
                $segments = request()->segments();
                $url = '';
                $excludeDigits = true; // Set this to true to exclude pure number segments
                $aliases = [
                    'home' => 'Home',
                    'categories' => 'Kategori Kepribadian',
                    'personality_types' => 'Tipe Kepribadian',
                    'quiestions' => 'Soal Test',
                    'results' => 'Hasil Kepribadian',
                    'universities' => 'Rekomendasi Universitas',
                    'edit' => 'Edit',
            ...
            <ol class="flex items-center whitespace-nowrap" aria-label="Breadcrumb">
                @foreach ($segments as $key => $segment)
                    @php
                        $url .= '/' . $segment;
                        $isLastSegment = $key === count($segments) - 1;
                        // Check if the segment is numeric and should be excluded
                        $isNumericSegment = $excludeDigits && is_numeric($segment);
                        $segmentFormatted = $aliases[$segment] ?? ucwords(str_replace(['-', '_'], ' ', $segment));
                    @endphp
                    ...
                    @if (!$isNumericSegment)
                        <li class="inline-flex items-center">
                            @if (!$isLastSegment)
                                <a href="{{ url($url) }}"
                                    class="flex items-center text-base text-gray-500 hover:text-indigo-600 focus:outline-none focus:text-indigo-600">
                                    {{ $segmentFormatted }}
                            ...
                            @else
                                <span class="inline-flex items-center text-base font-semibold text-gray-800 truncate"
                                    aria-current="page">
                                    {{ $segmentFormatted }}
                                </span>
                            @endif

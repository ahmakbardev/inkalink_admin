...
@section('content')
    <div class="bg-indigo-600 px-8 pt-10 lg:pt-14 pb-16 flex justify-between items-center mb-3">
        <!-- title -->
        <h1 class="text-xl text-white">Tipe Kepribadian</h1>
        <a href="{{ route('personality_types.create') }}"
                ...
                <table class="text-left w-full whitespace-nowrap">
                    ...
                    <tbody>
                        @if ($personalityTypes->isEmpty())
                            <tr>
                                <td colspan="5" class="text-center py-3 border-b border-gray-300">Tidak ada data tipe
                                    kepribadian.</td>
                            </tr>
                        @else
                            @foreach ($personalityTypes as $index => $type)
                                <tr>
                                    <td class="border-b border-gray-300 py-3 px-6">{{ $index + 1 }}</td>
                                    <td class="border-b border-gray-300 py-3 px-6">{{ $type->name }}</td>
                                    <td class="border-b border-gray-300 py-3 px-6">
                                        <div class="truncate w-64">{{ $type->description }}</div>
                                    </td>
                                    <td class="border-b border-gray-300 py-3 px-6">
                                        @if ($type->image)
                                    ...
                                    <td class="border-b border-gray-300 py-3 px-6">
                                        <a href="{{ route('personality_types.edit', $type->id) }}"
                                        ...
                                        <button type="button"
                                            ...
                                            data-url="{{ route('personality_types.destroy', $type->id) }}"
                                            data-bs-toggle="tooltip" data-bs-placement="top"
                                            data-bs-title="Hapus Tipe Kepribadian">
    ...
    @include('contents.tipe-kepribadian.components.delete-modal')

...
@section('content')
    <div class="bg-indigo-600 px-8 pt-10 lg:pt-14 pb-16 flex justify-between items-center mb-3">
        <h1 class="text-xl text-white">Hasil Kepribadian</h1>
            ...
            <div class="relative overflow-x-auto" data-simplebar style="max-height: 600px">
                <table class="text-left w-full whitespace-nowrap">
                    ...
                    <tbody>
                        @if ($results->isEmpty())
                            <tr>
                                <td colspan="5" class="text-center py-3 border-b border-gray-300">Tidak ada data hasil
                                    kepribadian.</td>
                            </tr>
                        @else
                                ...
                                <tr>
                                    ...
                                    <td class="border-b border-gray-300 py-3 px-6">
                                        @foreach ($result->category_ids as $categoryId)
                                            {{ \App\Models\Category::find($categoryId)->name }}{{ !$loop->last ? ', ' : '' }}
                                        @endforeach
                                    </td>
                                    <td class="border-b border-gray-300 py-3 px-6">{{ $result->code }}</td>
                                    <td class="border-b border-gray-300 py-3 px-6">
                                        <div class="truncate w-64">{{ $result->description }}</div>
                                    </td>
                                    <td class="border-b border-gray-300 py-3 px-6">
                                        ...
                                        <button type="button"
                                            ...
                                            data-url="{{ route('results.destroy', $result->id) }}" data-bs-toggle="tooltip"
                                            data-bs-placement="top" data-bs-title="Hapus Hasil Kepribadian">
    ...
    @endif
    @include('contents.results.components.delete-modal')
